...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        lm_weights = get_lm_weights(input_target_ids, word2id, ['[PAD]', '[UNK]', '[CLS]', '[SEP]', '[MASK]'])
        ...
        masked_lm_weights = get_lm_weights(masked_lm_ids, word2id, ['[PAD]', '[UNK]'])
        ...
        (lm_loss, lm_example_loss, lm_log_probs) = get_lm_output(
            bert_config, model.get_origin_sequence_output(), model.get_embedding_table(), input_target_ids,
            lm_weights)
        ...
        total_loss = masked_lm_loss + next_sentence_loss + masked_lm_type_loss + lm_loss
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(masked_lm_example_loss, masked_lm_log_probs, masked_lm_ids,
                          ...
                          next_sentence_log_probs, next_sentence_labels, lm_example_loss, lm_log_probs,
            ...
            wrong_mask_lm_label = tf.constant(value=-1, dtype=tf.int32, shape=masked_lm_ids.shape)
            unk_id = word2id['[UNK]']
            unk_tf = tf.constant(value=unk_id, dtype=tf.int32, shape=masked_lm_ids.shape)
            condition_mask_lm_tf = tf.equal(masked_lm_ids, unk_tf)
            new_mask_lm_labels = tf.where(condition_mask_lm_tf, wrong_mask_lm_label, masked_lm_ids)

            wrong_lm_label = tf.constant(value=-1, dtype=tf.int32, shape=input_target_ids.shape)
            unk_tf = tf.constant(value=unk_id, dtype=tf.int32, shape=input_target_ids.shape)
            condition_lm_tf = tf.equal(input_target_ids, unk_tf)
            new_lm_labels = tf.where(condition_lm_tf, wrong_lm_label, input_target_ids)
...
def get_masked_lm_output(bert_config, input_tensor, output_weights, output_type_weights, positions,
    ...
    with tf.variable_scope("cls/predictions"):
        ...
        logits_type = tf.matmul(input_tensor, output_type_weights, transpose_b=True)
        ...
        # padding predictions.
        type_per_example_loss = -tf.reduce_sum(log_probs_type * one_hot_type_labels, axis=[-1])
    ...
    with tf.variable_scope("cls/predictions/addtype"):
        ...
        output_bias = tf.get_variable(
            ...
            initializer=tf.zeros_initializer())
        logits = tf.matmul(concat_input_tensor, output_weights, transpose_b=True)
